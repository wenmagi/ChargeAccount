...
final class DateTimeParser {
    ...
    private static final Integer NUM_DIGITS = /*Integer.valueOf(NUM_DIGITS_FOR_FRACTIONAL_SECONDS);*/LangUtils.parseInt(NUM_DIGITS_FOR_FRACTIONAL_SECONDS);
    ...
    private void parseDate(String aDate) {
        ...
        if (matcher.matches()) {
            ...
            if (year != null) {
                fYear = LangUtils.parseInt(year);
            }
            String month = getGroup(matcher, 2, 5);
            if (month != null) {
                fMonth = LangUtils.parseInt(month);
            }
            String day = getGroup(matcher, 3);
            if (day != null) {
                fDay = LangUtils.parseInt(day);
            }
        } else {
    ...
    private void parseTime(String aTime) {
        ...
        if (matcher.matches()) {
            ...
            if (hour != null) {
                fHour = LangUtils.parseInt(hour);
            }
            String minute = getGroup(matcher, 2, 6, 9);
            if (minute != null) {
                fMinute = LangUtils.parseInt(minute);
            }
            String second = getGroup(matcher, 3, 7);
            if (second != null) {
                fSecond = LangUtils.parseInt(second);
            }
            String decimalSeconds = getGroup(matcher, 4);
            if (decimalSeconds != null) {
                fNanosecond = LangUtils.parseInt(convertToNanoseconds(decimalSeconds));
            }
        } else {
